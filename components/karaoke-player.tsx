...
const KaraokePlayer: React.FC<KaraokePlayerProps> = ({ videos, title }) => {
  ...
  const handleReplay = () => {
    if (activeVideo === "original" && videoRef.current) {
      setActiveVideo("original");
      videoRef.current.play();
    } else if (activeVideo === "karaoke" && videoRef.current) {
      setActiveVideo("karaoke");
      videoRef.current.play();
    }
    setIsPlaying(true);
  ...
  const togglePlayPause = () => {
    if (isPlaying) {
      if (activeVideo === "original" && videoRef.current)
        videoRef.current.pause();
      if (activeVideo === "karaoke" && videoRef.current)
        videoRef.current.pause();
      setIsPlaying(false);
    } else {
      if (activeVideo === "original" && videoRef.current) {
  ...
  return (
    ...
    >
      <div className="relative aspect-video bg-black">
        ...
        >
          <source
            src={activeVideo === "original" ? videos.original : videos.karaoke}
            type="video/mp4"
          />
        </video>
      </div>
